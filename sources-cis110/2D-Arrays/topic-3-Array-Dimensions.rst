...
.. note::
     ...
     Note that length is a field and not a method, so you don't add parentheses after length. When a  two-dimensional array is rectangular (it has the same number of columns in each row) you can just use the length of the first inner array as the number of columns as shown by ``ticketInfo[0].length``. Be careful, though: this doesn't work for ragged arrays!
